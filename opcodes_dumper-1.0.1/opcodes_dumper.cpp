...
#include <cstdio>
#include "/home/keything/php-5.6.25/sapi/embed/php_embed.h"
...
char * format_znode(zend_uchar type, znode_op *n){
	static char buffer[BUFFER_LEN];

	switch (type) {
		case IS_CONST: //1
			snprintf(buffer, BUFFER_LEN, "$%d", n->constant);
            return buffer;
			break;
		case IS_VAR: //4
...
void dump_op(zend_op *op, int num){
	printf("%5d  %5d %20s %020s %020s %020s\n", num, op->lineno,
			opname(op->opcode),
			format_znode(op->op1_type, &op->op1),
			format_znode(op->op2_type, &op->op2),
			format_znode(op->result_type, &op->result)) ;
}

void dump_op_array(zend_op_array *op_array){
	if(op_array) {
		int i;
		printf("%5s %5s %20s %020s %020s %020s\n", "opnum", "line", "opcode", "op1", "op2", "result");
